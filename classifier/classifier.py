...
class ClassifierModel:
    def __init__(self, classifier_name, n_features, n_classes, LR_max_iter = 10000, device="cpu"):
        self.classifier_name = classifier_name
        self.n_features = n_features
        self.n_classes = n_classes
        self.device = device
        if self.classifier_name == "logistic_regression":
            self.model = LogisticRegression(max_iter = LR_max_iter)
    ...
    def eval(self, x, y, f1_average = 'macro'):
        if self.classifier_name in ["logistic_regression", "random_forest", "svm", "xgboost"]:
            y_pred = self.model.predict(x)
            accuracy = accuracy_score(y, y_pred)
            f1 = f1_score(y, y_pred, average=f1_average)
