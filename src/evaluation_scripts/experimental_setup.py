...
sys.path.insert(0,'C:\Programming\MAAL_Code') ##PLEASE INSERT YOUR PATH HERE FOR IMPORTING FROM SRC
...
def run(results_path, data_set, dataset_dir, query_strategy, budget, test_ratio, seed):
    """
    ...
    X, y_true, y, index_frame = load_data(data_set_name=data_set)
    ...
    X_train, X_test, y_true_train, y_true_test, y_train, y_test,index_frame_train, index_frame_test = train_test_split(X, y_true, y, index_frame, test_size=test_ratio,
                                                                                   random_state=seed)
    while not np.array_equal(np.unique(y_true_train), np.unique(y_true_test)):
        X_train, X_test, y_true_train, y_true_test, y_train, y_test, index_frame_train, index_frame_test = train_test_split(X, y_true, y,index_frame, random_state=seed,
                                                                                       test_size=test_ratio)
        seed += 1000
        print('new seed: {}'.format(seed))
    n_samples = len(X_train)

    dataset_dir = os.path.join("data_processing",dataset_dir)


    with open(dataset_dir+"/X_train", "wb") as fp:   #Pickling
       pickle.dump(X_train, fp)
    with open(dataset_dir+"/X_test", "wb") as fp:   #Pickling
       pickle.dump(X_test, fp)
    with open(dataset_dir+"/y_true_train", "wb") as fp:   #Pickling
       pickle.dump(y_true_train, fp)
    with open(dataset_dir+"/y_true_test", "wb") as fp:   #Pickling
       pickle.dump(y_true_test, fp)
    with open(dataset_dir+"/y_train", "wb") as fp:   #Pickling
       pickle.dump(y_train, fp)
    with open(dataset_dir+"/y_test", "wb") as fp:   #Pickling
       pickle.dump(y_test, fp)
    with open(dataset_dir+"/index_frame_train", "wb") as fp:   #Pickling
       pickle.dump(index_frame_train, fp)
    with open(dataset_dir+"/index_frame_test", "wb") as fp:   #Pickling
    ...
    with open(dataset_dir+"/budget", "wb") as fp:   #Pickling
    ...
    for b in range(budget):
        ...
        sample_index = index_frame_train[sample_id]

        instances.append(sample_index)
        if sample_index in instance_annotator_pair :
            instance_annotator_pair[sample_index].append(annotator_id[0])
        else :
            instance_annotator_pair[sample_index] = [annotator_id[0]]
    ...
    with open(dataset_dir+"/MAPAL_results_path", "wb") as fp:   #Pickling
       pickle.dump(results_dataframe_path, fp)
    with open(dataset_dir+"/instance_annotator_pair", "wb") as fp:   #Pickling
       pickle.dump(instance_annotator_pair, fp)
    with open(dataset_dir+"/instances", "wb") as fp:   #Pickling
...
def main():
    ...
    run(results_path=args.results_path, data_set=args.data_set, dataset_dir=args.dir_name ,query_strategy=args.query_strategy, budget=args.budget,
