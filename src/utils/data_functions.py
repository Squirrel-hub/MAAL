...
def load_data(data_set_name):
    ...
    try:
        ...
        relative_path = '../../datasets/' + data_set_name + '.csv'
        ...
        print('TYPE dataset : ',type(data_set))
        index_frame = list(data_set.index.values)
        ...
        relative_path = '../../datasets/data_set_ids.csv'
        data_set = pd.read_csv(os.path.join(abs_path, relative_path))
    ...
    return X, y_true, y, index_frame


def investigate_data_set(data_set_name):
    X, y_true, y, index_frame = load_data(data_set_name)
